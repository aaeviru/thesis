...
\end{abstract}
 ...
 類似度攻撃(SimAtt)\cite{simattack2016}は質問者が提出した質問と攻撃者が事前に得た質問者のプロフィール間の類似度を計算し，
 この類似度を用い質問者の新の質問を見破る．
 Simattでは単語ベクトルで質問を表し，
 質問のプロフィールを質問の集合とする．
 アルゴリズム\ref{a:simatt}が質問$q$と質問者のプロフィール$P_u$間の類似度$sim_{q,P_u}$の計算方法を示す．
 ...
 SimAttは質問グループの中質問者のプロフィールと一番類似度が高い質問が真の質問であると判定する．
