...
\end{abstract}
 ...
 文章数$M \times$単語数$N$の行列$G$でコーパスを記述できる．
 潜在的意味解析(LSA)は行列$G$を特異値分解(SVD)し低ランク近似し，より高い圧縮を得る．
 ...
 単語$\times$文書行列$G$の$(i,j)$番目の要素は単語$w_i$のが文章$d_j$における$tf\text{-}idf$値である．
 $G$を特異値分解$G = USV^T$し、$U$、$S$、$V$ の各列ベクトルを特異値が大きい順に$K$個用いて$G$の低ランク近似$G_K=U_KS_KV_{K}^T$を得る．
 このように低ランク分解によって、単語とトピックの関係を分析できる．
 $G_K$の$(i,j)$番目の要素は$i$番目の単語と$j$番目のトピックの関係を表す.
 その値が大きければ大きいほど単語とピックの関係が強い．
 単語$w_i$と対応する行列$G_K$の行$\ell_i$を単語$w_i$のトピックベクトルとし，
 \begin{equation}
 rscore_{LSA}(q,t_j) = \sum_{w_i \in q}\ell_i[j]
 \end{equation}
 によって質問$q$とトピック$t_j$の関連性を計算する．
 
 本論文では単語$\times$文書行列$G$の代わりに単語$\times$国際特許分類行列$G'$を用いる．
 単語$\times$国際特許分類行列$G'$の$(i,j)$番目の要素は単語$w_i$の国際特許分類$t_k$における$tf\text{-}idf$値である．
 ...
 本節ではダミー質問を混ぜた質問ログのみから真の質問を見つかる攻撃手法を提案する．
 ...
 アルゴリズム\ref{a:simatt2}が意味的近い一連の質問を取り出す攻撃手法を示す．
 \begin{algorithm}
 \caption{類似度攻撃}
 \begin{algorithmic}[1]
 ...
 \begin{defi}{単語ベクトル．}
  ...
  \begin{aligned}
   ...
   & \forall w \in wvec_{SA}(t), w \in W \\
   & \forall 1 \leq i < j \leq |W|,w_i \neq w_j,rscore(w_i,t) \geq rscore(w_j,t) 
  \end{aligned}
  \end{equation}
 \end{defi}
 ...
 一方，単語ベクトルに同じ順番を持つ単語がその単語ベクトルを持つトピックに対して同じ様な関連性を持つと考えられる．
 また，同じ数字ベクトルで表わせる質問も その質問が属するトピックに対して同じ様な関連性を持つと考えられる．
